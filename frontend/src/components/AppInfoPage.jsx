...
import { Info, Tag, Zap, Star, ChevronDown, ChevronUp, X } from 'lucide-react';

const API = process.env.REACT_APP_BACKEND_URL;
const LS_KEY = 'sv_last_seen_version';

// ── Version Changelog Card (also used as popup) ───────────────────────────────
export const VersionCard = ({ data, onDismiss, isPopup = false }) => {
  ...
  return (
    <div
      className={`${isPopup ? 'fixed inset-0 z-[200] flex items-start justify-center pt-16 px-4' : ''}`}
      style={isPopup ? { background: 'rgba(0,0,0,0.80)', backdropFilter: 'blur(8px)' } : {}}
      onClick={isPopup ? onDismiss : undefined}
    >
      <div
        className="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl"
        style={{ background: 'linear-gradient(160deg,#1e0f40 0%,#120828 100%)', border: '1px solid rgba(139,92,246,0.3)' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Version header */}
        <div className="px-6 pt-6 pb-4" style={{ background: 'linear-gradient(90deg,#2d1b69,#1a0a3d)' }}>
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Tag className="w-4 h-4 text-purple-300" />
                <span className="text-purple-300/60 text-xs font-mono uppercase tracking-widest">StreamVault</span>
              </div>
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto space-y-5">
          {sections && sections.length > 0 ? (
            sections.map((section, si) => (
              <div key={si}>
                <h3 className="text-purple-300 text-sm font-semibold mb-2 flex items-center gap-2">
                  {si === 0 ? <Zap className="w-4 h-4" /> : <Star className="w-4 h-4" />}
                  {section.heading}
                </h3>
            ...
            <div className="text-center py-6">
              <p className="text-purple-300/40 text-sm">No changelog entries yet.</p>
              <p className="text-purple-300/30 text-xs mt-1">Admin can add release notes via the dashboard.</p>
            </div>
          )}
        </div>
...
export const VersionPopup = () => {
  ...
  const dismiss = useCallback(() => {
    if (versionData?.version) {
      localStorage.setItem(LS_KEY, versionData.version);
    }
    setShow(false);
  }, [versionData]);
...
const AppInfoPage = () => {
  ...
  useEffect(() => {
    axios.get(`${API}/api/app-version`)
      ...
      .catch(() => setVersionData({ version: '0.94.0.1.A (Alpha build)', sections: [] }))
    ...
    <div className="flex-1 overflow-y-auto" style={{ background: '#0d0820' }}>
      ...
      <div className="max-w-2xl mx-auto px-8 py-10">
        <div className="mb-8">
          ...
          <p className="text-purple-300/60 text-sm">Version details and release notes</p>
        </div>
          ...
          <>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl border border-purple-800/50 text-purple-300 hover:bg-purple-900/20 text-sm font-medium transition-colors"
              >
                Show version popup
              </button>
              <button
                onClick={() => { localStorage.removeItem(LS_KEY); }}
                className="px-4 py-2.5 rounded-xl border border-purple-900/30 text-purple-400/60 hover:text-purple-300 text-sm transition-colors"
