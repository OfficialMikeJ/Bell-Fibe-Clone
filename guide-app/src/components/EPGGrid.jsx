...
const EPGGrid = ({ channels, programs, timeSlots, selectedChannelId, onChannelSelect }) => {
  ...
  const calculateProgramWidth = (duration) => (duration / 30) * 160;

  const getProgramsForChannel = (channelId) =>
    programs.filter(p => p.channel_id === channelId).sort((a, b) =>
  ...
  const getLogoSrc = (channel) => {
    ...
    return `${API_URL}/api${channel.logo_path}`;
  };
      ...
      <div className="space-y-1.5">
        {channels.map((channel, idx) => {
          ...
          return (
            ...
            >
              {/* Channel info */}
              <div className={`w-52 flex-shrink-0 flex items-center gap-2 px-3 py-2 bg-[#2a2a2a] rounded-l-lg ${
              ...
              {/* Programs */}
              <div className="flex-1 flex items-center bg-[#2a2a2a] rounded-r-lg overflow-hidden">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            {program.poster_path && (
                              <img src={`${API_URL}/api${program.poster_path}`} alt=""
