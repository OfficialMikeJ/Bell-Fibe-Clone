...
const ChannelFeatured = ({ channel, currentProgram }) => {
  ...
  const videoSrc = currentProgram?.media_file_path
    ? `${API_URL}/api${currentProgram.media_file_path}`
    : channel?.stream_url || null;

  const posterSrc = currentProgram?.poster_path
    ? `${API_URL}/api${currentProgram.poster_path}`
    : null;
  ...
  const handlePlayPause = () => {
    ...
    isPlaying ? videoRef.current.pause() : videoRef.current.play().catch(() => setHasError(true));
  ...
  const logoSrc = channel.logo_path
    ? (channel.logo_path.startsWith('http') ? channel.logo_path : `${API_URL}/api${channel.logo_path}`)
    : null;
          ...
          {videoSrc && !hasError ? (
            <>
              <video ref={videoRef} className="w-full h-full object-cover"
                src={videoSrc} poster={posterSrc || undefined}
